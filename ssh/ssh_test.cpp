...
large_uint parse_ssh_mpint(const std::vector<uint8_t>& v) {
    util::buffer_view b(v.data(), v.size());
    return ssh::string_to_int<large_uint>(ssh::get_string(b));
}
...
int main()
{
    ...
    FUNTLS_ASSERT_EQUAL(large_uint("0")                  , parse_ssh_mpint({0x00,0x00,0x00,0x00}));
    FUNTLS_ASSERT_EQUAL(large_uint(1)                    , ssh::string_to_int<large_uint>({0x01}));
    FUNTLS_ASSERT_EQUAL(large_uint("0x9a378f9b2e332a7")  , parse_ssh_mpint({0x00,0x00,0x00,0x08,0x09,0xa3,0x78,0xf9,0xb2,0xe3,0x32,0xa7}));
    FUNTLS_ASSERT_EQUAL(large_uint("0x80")               , parse_ssh_mpint({0x00,0x00,0x00,0x02,0x00,0x80}));
    //FUNTLS_ASSERT_EQUAL(large_int("-0x1234")            , parse_ssh_mpint({0x00,0x00,0x00,0x02,0xed,0xcc}));
    //FUNTLS_ASSERT_EQUAL(large_int("-0xdeadbeef")        , parse_ssh_mpint({0x00,0x00,0x00,0x05,0xff,0x21,0x52,0x41,0x11}));
    // Illegal zero repr
    FUNTLS_ASSERT_THROWS(ssh::string_to_int<large_uint>({0x00}), std::runtime_error);
    FUNTLS_ASSERT_THROWS(ssh::string_to_int<large_uint>({0x00,0x00,0x00,0x00}), std::runtime_error);
