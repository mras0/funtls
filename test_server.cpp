...
public:
    using ptr_t = std::shared_ptr<connection_to_client>;
    ...
    static ptr_t make(const std::string& name, std::unique_ptr<stream> stream, const std::vector<const server_id*> server_ids, const client_connect_handler& on_client_connected) {
        auto c = ptr_t(new connection_to_client{name, std::move(stream), server_ids, on_client_connected});
        c->read_client_hello();
        return c;
    }
        ...
        acceptor.async_accept(state->socket,
            [state, &start_accept, &server_id] (const boost::system::error_code& ec) {
                if (ec) {
                    ...
                    tls::connection_to_client::make(oss.str(), make_tls_stream(std::move(state->socket)), {server_id.get()}, &main_loop);
                }
                start_accept();
            });
    };
