...
public:
    static void make(std::unique_ptr<stream> stream, const std::vector<const server_id*> server_ids, const client_connect_handler& on_client_connected, std::ostream* log) {
        auto c = std::shared_ptr<connection_to_client>(new connection_to_client{std::move(stream), server_ids, on_client_connected, log});
        c->read_client_hello();
    }
...
private:
    ...
    connection_to_client(std::unique_ptr<stream> stream, const std::vector<const server_id*> server_ids, const client_connect_handler& on_client_connected, std::ostream* log);
...
{
    connection_to_client::make(std::move(stream), server_ids, on_client_connected, log);
}

connection_to_client::connection_to_client(std::unique_ptr<stream> stream, const std::vector<const server_id*> server_ids, const client_connect_handler& on_client_connected, std::ostream* log)
    ...
    , server_kex_(nullptr)
{
    assert(log_ && "null log not supported yet");
